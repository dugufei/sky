...
public class SKToast {
	...
	public void show(final int msg) {
		...
		if (isMainLooper) {
			SKHelper.executors().mainThread().execute(new Runnable() {
				...
				@Override public void run() {
					showToast(SKHelper.getInstance().getString(msg), Toast.LENGTH_SHORT);
				}
			});
		} else {
			showToast(SKHelper.getInstance().getString(msg), Toast.LENGTH_SHORT);
	...
	public void show(final int msg, final int duration) {
		...
		if (isMainLooper) {
			SKHelper.executors().mainThread().execute(new Runnable() {
				...
				@Override public void run() {
					showToast(SKHelper.getInstance().getString(msg), duration);
				}
			});
		} else {
			showToast(SKHelper.getInstance().getString(msg), duration);
	...
	protected void showToast(String text, int duration) {
		if (mToast == null) {
			mToast = Toast.makeText(SKHelper.getInstance(), text, duration);
		} else {
			mToast.setText(text);
