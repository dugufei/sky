...
/**
 * @author sky
 * @version 1.0 on 2018-08-02 下午5:33
 * @see OKHttpCallExtend
 */
public class OKHttpCallExtend<T> implements SKCall<T> {
	...
	@GuardedBy("this") private @Nullable Throwable		creationFailure;	// Either a RuntimeException or IOException.

	@GuardedBy("this") private boolean					executed;

	OKHttpCallExtend(ServiceMethod<T, ?> serviceMethod, @Nullable Object[] args) {
		this.serviceMethod = serviceMethod;
		this.args = args;
	}

	@SuppressWarnings("CloneDoesntCallSuperClone") // We are a final type & this saves clearing state.
	@Override public OKHttpCallExtend<T> clone() {
		return new OKHttpCallExtend<>(serviceMethod, args);
	}
	...
	@Override public void enqueue(final Callback<T> callback) {
		...
		call.enqueue(new okhttp3.Callback() {
			...
			@Override public void onFailure(okhttp3.Call call, IOException e) {
				try {
					callback.onFailure(OKHttpCallExtend.this, e);
				} catch (Throwable t) {
					t.printStackTrace();
				}
			}
			...
			private void callSuccess(Response<T> response) {
				try {
					callback.onResponse(OKHttpCallExtend.this, response);
				} catch (Throwable t) {
					t.printStackTrace();
				}
			}
	...
	@Override public T get() {
		...
		try {
			Response<T> response = skyCall.execute();
			if (!response.isSuccessful()) {
				...
				throw new SKHttpException(stringBuilder.toString());
			}

			return response.body();
		} catch (IOException e) {
			throw new SKHttpException("网络异常", e.getCause());
		}
	}
