...
public class SKYDownloadRequestQueue {
	 ...
	 * 请求集合
	 */
	private Set<SKYBaseRequest>						mCurrentRequests					= new HashSet<>();

	/** 阻塞队列. */
	private PriorityBlockingQueue<SKYBaseRequest>	mDownloadQueue						= new PriorityBlockingQueue<>();
	 ...
	 *            工作线程数量
	 */
	public SKYDownloadRequestQueue(int threadPoolSize) {
		okHttpClient = getOkHttpClient();
	...
	int query(int downloadId) {
		...
		return SKYDownloadManager.STATUS_NOT_FOUND;
	}
