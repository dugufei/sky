...
public final class SKYBuilder {
	...
	public void layoutLoading() {
		...
		if (layoutLoading == null && vsLoading != null) {
			...
			checkNotNull(layoutLoading, "无法根据布局文件ID,获取layoutLoading");
		}
	...
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH) private void createLayout() {
		...
		if (getLayoutId() > 0) {
			...
			if (layoutStateId > 0) {
				...
				if (layoutStateContent == null) {
					checkNotNull(layoutContent, "指定切换状态布局后,内容不能为空");
				}
			}
			checkNotNull(layoutContent, "无法根据布局文件ID,获取layoutContent");
		...
		if (layoutEmptyId > 0) {
			...
			checkNotNull(layoutEmpty, "无法根据布局文件ID,获取layoutEmpty");
		...
		if (layoutBizErrorId > 0) {
			...
			checkNotNull(layoutBizError, "无法根据布局文件ID,获取layoutBizError");
			if (layoutStateId > 0) {
				ViewGroup view = layoutContent.findViewById(layoutStateId);
		...
		if (layoutHttpErrorId > 0) {
			checkArgument(layoutHttpErrorId > 0, "网络错误布局Id不能为空,重写公共布局Application.layoutBizError 或者 在Buider.layout里设置");
			layoutHttpError = mInflater.inflate(layoutHttpErrorId, null, false);
			checkNotNull(layoutHttpError, "无法根据布局文件ID,获取layoutHttpError");
			if (layoutStateId > 0) {
				ViewGroup view = layoutContent.findViewById(layoutStateId);
		...
		if (getRecyclerviewId() > 0) {
			...
			if (SKYRVAdapter != null) {
				...
				if (isHeaderFooter) {
					...
					checkNotNull(gridLayoutManager, "LayoutManger，不是GridLayoutManager");
				...
				checkNotNull(null, "SKYRVAdapter适配器不能为空");
			}
